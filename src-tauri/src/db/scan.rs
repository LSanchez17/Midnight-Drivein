...
use crate::{
    ...
    db::types::{MatchSummary, ScanResult},
    utils::naming::{normalize_filename, normalize_for_match},
};
...
fn assign_best_matches(
    mut candidates: Vec<(f64, String, String)>,
...
pub(crate) async fn scan_library(
    ...
    let movie_walk =
        tokio::task::spawn_blocking(move || walk_folder(movies_folder_clone, "movies"))
            .await
            .map_err(|e| {
...
mod tests {
    ...
    async fn scan_library_is_idempotent() {
        ...
        scan_library(&pool, &flag()).await.unwrap();
        scan_library(&pool, &flag()).await.unwrap();
    ...
    async fn get_scan_summary_returns_none_before_scan() {
        let pool = setup().await;
        let result = get_scan_summary(&pool).await.unwrap();
    ...
    async fn get_scan_summary_returns_result_after_scan() {
        ...
        let summary = get_scan_summary(&pool).await.unwrap();
    ...
    async fn match_media_files_underscore_title() {
        let pool = setup().await;
        let slot_id = setup_movie_slot(&pool, "ep-1", "a", "Castle Freak", None).await;
        setup_media_file(&pool, "mf-1", "castle_freak_1995.mkv", "movies").await;
    ...
    async fn match_media_files_alias_exact() {
        ...
        setup_movie_slot(
            ...
            "a",
            "Castle Freak",
            Some(&["castle freak 95"]),
        )
        .await;

        // File normalises to "castle freak 95"
        setup_media_file(&pool, "mf-1", "castle_freak_95.mkv", "movies").await;
        ...
        assert_eq!(
            row.0, 1.0,
            "exact alias match should force confidence to 1.0"
        );
    }
    ...
    async fn match_media_files_uniqueness() {
        ...
        setup_movie_slot(&pool, "ep-1", "a", "Castle Freak", None).await;
        setup_movie_slot(&pool, "ep-2", "a", "Castle Freak 2", None).await;
        setup_media_file(&pool, "mf-1", "Castle Freak (1995).mkv", "movies").await;
    ...
    async fn match_media_files_respects_user_override() {
        let pool = setup().await;
        let slot_id = setup_movie_slot(&pool, "ep-1", "a", "Castle Freak", None).await;
        setup_media_file(&pool, "mf-user", "user_choice.mkv", "movies").await;
        setup_media_file(&pool, "mf-auto", "Castle Freak (1995).mkv", "movies").await;
        ...
        assert_eq!(
            row.0.as_deref(),
            Some("mf-user"),
            "user override must not be replaced"
        );
        assert_eq!(row.1, 1);
    }
